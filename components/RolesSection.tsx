          ...
          {roleOptions.map((role) => (
            <button
              ...
              className={`rounded-full border px-4 py-2 text-sm font-semibold transition ${
                selectedRole === role
                  ? "border-[#C17A3A] bg-[#FFF0E1] text-[#8A531E]"
                  : "border-[#DDD6CD] bg-white text-[#1B2A4A] hover:border-[#C17A3A]"
              }`}
            >
              {role}
            </button>
        ...
        <form ref={formRef} action={formAction} className="mt-6 space-y-4">
          ...
          <div className="flex flex-col gap-3 pt-1 sm:flex-row">
            <button
              type="submit"
              disabled={pending}
              className="inline-flex items-center justify-center rounded-md bg-[#D28A3D] px-7 py-3 font-semibold text-white transition hover:bg-[#bf7f36] disabled:cursor-not-allowed disabled:opacity-70"
            >
          ...
          <p
            aria-live="polite"
            className={`text-sm ${status === "success" ? "text-[#15803D]" : "text-[#6B7280]"}`}
          >
            {message}
          </p>
        </form>
